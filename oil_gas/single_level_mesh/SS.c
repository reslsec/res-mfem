...
build_mat_vec(DOF *u_o, DOF *dp, DOF *p_h, DOF *ds, DOF *s_o, DOF *s_o_l, DOF *mu_o, DOF *b_o, DOF *b_o_l, DOF *kro, DOF *dot_kro, DOF *phi, DOF *phi_l, DOF *dot_phi, DOF *dot_bo, DOF *dot_muo, DOF *Rs, DOF *Rs_l, DOF *dot_Rs, DOF *q_o, DOF *mu_g, DOF *b_g, DOF *b_g_l, DOF *krg, DOF *dot_krg, DOF *dot_bg, DOF *dot_mug, DOF *q_g, MAP *map_u, MAP *map_p, MAT *A, MAT *B, MAT *TB, MAT *C, VEC *vec_f, VEC *vec_g)
{
	...
	ForAllElements(g, e){
		...
		alpha_g = K * p_krg[0] * p_bg[0] * p_mug[0] + K * p_krg[0] * (p_mug[0] * p_dotbg[0] + p_dotmug[0] * p_bg[0]) * p_dp[0] + K * p_bg[0] * p_mug[0] * p_dotkrg[0] * p_ds[0]; 
		alpha_o = K * p_kro[0] * p_bo[0] * p_muo[0] + K * p_kro[0] * (p_muo[0] * p_dotbo[0] + p_dotmuo[0] * p_bo[0]) * p_dp[0] + K * p_bo[0] * p_muo[0] * p_dotkro[0] * p_ds[0]; 
...
//		phgPrintf("Average time of AMG---------%lfs\n", time_amg / nits_uzawa);
		phgPrintf("Avearge iter of AMG---------%d\n", nits_amg);
