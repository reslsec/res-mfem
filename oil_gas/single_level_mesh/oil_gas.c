...
/*build_linear_system*/
static void
build_mat_vec(DOF *u_o, DOF *p_h, DOF *p_nk, DOF *s_o, DOF *s_o_l, DOF *mu_o, DOF *b_o, DOF *b_o_l, DOF *kro, DOF *phi, DOF *phi_l, DOF *dot_phi, DOF *dot_bo, DOF *Rs, DOF *Rs_l, DOF *dot_Rs, DOF *q_o, DOF *mu_g, DOF *b_g, DOF *b_g_l, DOF *krg, DOF *dot_bg, DOF *q_g, MAP *map_u, MAP *map_p, MAT *A, MAT *B, MAT *TB, MAT *C, VEC *vec_f, VEC *vec_g)
{
	GRID *g = u_o->g;
	SIMPLEX *e;
	...
	ForAllElements(g, e){
		...
		for (i = 0; i < M; i++){
			...
			phgQuadDofTimesBas(e, p_nk, p_h, i, QUAD_DEFAULT, &quad_pnew);
...
static void
Solve_Pressure(DOF *u_o, DOF *p_h, DOF *p0_h, DOF *q_o, DOF *s_o, DOF *s_o_l, DOF *q_g, DOF *p_nk, DOF *phi_l, DOF *b_o_l, DOF *b_g_l, DOF *Rs_l)
{
		GRID *g = u_o->g;	
	     MAT *A, *B, *TB, *C;
	     VEC *vec_f, *vec_g;
		...
		build_mat_vec(u_o, p_h, p_nk, s_o, s_o_l, mu_o, b_o, b_o_l, kro, phi, phi_l, dot_phi, dot_bo, Rs, Rs_l, dot_Rs, q_o, mu_g, b_g, b_g_l, krg, dot_bg, q_g, map_u, map_p, A, B, TB, C, vec_f, vec_g);
		phgDofFree(&phi);
		phgDofFree(&mu_o);
		phgDofFree(&b_o);
		phgDofFree(&kro);
...
static void
build_oileqn_so(SOLVER *solver, DOF *s_o, DOF *s_o_l, DOF *div_uo, DOF *q_o, DOF *p_h, DOF *p_nk, DOF *b_o, DOF *phi, DOF *dot_phi, DOF *dot_bo, DOF *phi_l, DOF *b_o_l)
{
	GRID *g = s_o->g;
	SIMPLEX *e;
	int i,j;
	...
	ForAllElements(g, e){
		...
		for (i = 0; i < N; i++){
			...
			phgQuadDofTimesBas(e, p_nk, s_o, i, QUAD_DEFAULT, &quad_pnew);
...
static void
Solver_oileqn_so(DOF *s_o, DOF *u_o, DOF *q_o, DOF *s_o_l, DOF *p_h, DOF *p_h0, DOF *p_nk, DOF *phi_l, DOF *b_o_l)
{
	GRID *g = s_o->g;
	SOLVER *solver;
	...
	b_o = phgDofNew(g, DOF_P0, 1, "b_o", DofNoAction);
	update_bo(p_h, b_o);
	build_oileqn_so(solver, s_o, s_o_l, div_uo, q_o, p_h, p_nk, b_o, phi, dot_phi, dot_bo, phi_l, b_o_l);
...
//	exit(1);
	...
	while (ctime < T -1e-8 ){
		...
		while (TRUE){
			...
			Solve_Pressure(u_o, p_h, p0_h, q_o, s_o, s_o_l, q_g, p_nk, phi_l, b_o_l, b_g_l, Rs_l);
			Solver_oileqn_so(s_o, u_o, q_o, s_o_l, p_h, p0_h, p_nk, phi_l, b_o_l);
