...
create_krg(DOF *s_o, DOF *krg)
{
	...
	ForAllElements(g, e){
		...
		p_krg[0] = KRG_SWC * pow((1. - p_so[0] - SGC) / (1. - SLC - SGC), NG); 
	}
}
void
create_dot_krg(DOF *s_o, DOF *dot_krg)
{
	GRID *g =s_o->g;
	SIMPLEX *e;
	FLOAT *p_so, *p_dotkrg;
	ForAllElements(g, e){
		...
		p_dotkrg[0] = NG * (-1. / (1. - SLC -SGC)) * KRG_SWC * pow((1. - p_so[0] - SGC) / (1. - SLC - SGC), NG -1); 
	}
}
void
create_kro(DOF *s_o, DOF *kro)
{
	...
	ForAllElements(g, e){
		...
		p_kro[0] = KRO_SGC * pow((p_so[0] - SLC) / (1. - SGC - SLC), NO); 
	}
}
void
create_dot_kro(DOF *s_o, DOF *dot_kro)
{
	GRID *g = s_o->g;
	SIMPLEX *e;
	FLOAT *p_so, *p_dotkro;
	ForAllElements(g, e){
		...
		p_dotkro[0] = NO * (1. / (1. - SLC - SGC)) * KRO_SGC * pow((p_so[0] - SLC) / (1. - SGC - SLC), NO-1); 
	}
}
void
