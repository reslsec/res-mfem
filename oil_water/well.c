...
void
Peaceman_Model(DOF *p_h, DOF *b_o, DOF *kro, FLOAT *pwf)
{
	GRID *g = p_h->g;
	SIMPLEX *e;
	FLOAT sum_lambda = 0, sum_vol = 0, sum_pwf = 0;
	FLOAT MU = 1e-9 / 3600., K = 1e-13;
	INT i;
	ForAllElements(g, e){
		if (e->region_mark == 1){
			FLOAT tmp = 0;
			...
			sum_lambda += phgQuadDofDotDof(e, kro, b_o, QUAD_DEFAULT);
		}
	}
#if USE_MPI
	...
	*pwf = *pwf - MU * PRODUCTION * (log(10. / 0.1) + 3.0 - 0.5) / (2. * M_PI * K * 10. * sum_lambda);
}
