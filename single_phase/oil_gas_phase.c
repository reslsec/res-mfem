...
build_mat_vec(DOF *u_o, DOF *p_h, DOF *p_h_newton, DOF *s_o, DOF *s_o_l, DOF *mu_o, DOF *b_o, DOF *b_o_l, DOF *kro, DOF *phi, DOF *phi_l, DOF *dot_phi, DOF *dot_bo, DOF *Rs, DOF *Rs_l, DOF *dot_Rs, DOF *q_o, DOF *mu_g, DOF *b_g, DOF *b_g_l, DOF *krg, DOF *dot_bg, DOF *q_g, MAP *map_u, MAP *map_p, MAT *A, MAT *B, MAT *TB, MAT *C, VEC *vec_f, VEC *vec_g)
{
	...
	ForAllElements(g, e){
		...
		for (i = 0; i < N; i++){
			for (j = 0; j < N; j++){
				mat_A[i][j] = stime * p_muo[0] * phgQuadKBasDotBas(e, u_o, i, u_o, j, QUAD_DEFAULT) / (p_kro[0] * p_bo[0]);
			}
		}
		for (i = 0; i < N; i++){
			for (j = 0; j < M; j++){
...
build_gaseqn_ug(SOLVER *solver, DOF *u_g, DOF *p_h, DOF *krg, DOF *b_g, DOF *mu_g, DOF *kro, DOF *b_o, DOF *mu_o)
{
	...
	ForAllElements(g, e){
		...
		for (i = 0; i < N; i++){
			I[i] = phgSolverMapE2L(solver, 0, e, i);
			for (j = 0; j < N; j++){
				   A[i][j] = phgQuadKBasDotBas(e, u_g, i, u_g, j, QUAD_DEFAULT) * p_mug[0] / (p_krg[0] * p_bg[0]);
			}
...
//	test_grid(p_h);
	...
	Get_Sg(s_o, s_g);
	sprintf(vtkfile, "oil_gas_%03d.vtk", "finaltime");
	phgExportVTK(g, vtkfile, p_h, s_o, s_g, q_o, q_g, u_o, NULL);
    phgDofFree(&p_h);
    phgDofFree(&p0_h);
    phgDofFree(&u_o);
    phgDofFree(&u_g);
