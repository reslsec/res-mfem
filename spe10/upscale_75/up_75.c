...
build_oil_so(SOLVER *solver, DOF *ds, DOF *div_uo, DOF *dp, PHASE *oil, PHASE *oil_l, MEDIUM *rock, MEDIUM *rock_l, COM_TRO *control)
{
	...
	ForAllElements(g, e){
		...
		FLOAT quad_qo = 0, quad_phil = 0, quad_phi = 0., quad_divuo = 0, quad_dp = 0 ;
		for (i = 0; i < N; i++){
...
static FLOAT
time_step(COM_TRO *control,DOF *dp, DOF *ds)
{
	FLOAT min = 1e6, dp_max = 1e-32, ds_max = 1e-32, tmp = 0;
	FLOAT coef = 1e6;
	tmp = control->dt;
	Find_Min_Max(dp, &min, &dp_max);
	Find_Min_Max(ds, &min, &ds_max);
	tmp = find_min(control->MAX_DS/ds_max, control->MAX_DP/dp_max);
	coef = find_min(tmp, 2);
	phgPrintf("dp_max = %le, ds_max = %le\n", dp_max, ds_max);
	phgPrintf("The Time step:  dt = %lf, coef = %lf\n", control->dt, coef);
	control->dt = coef * control->dt;
	...
	ForAllElements(g, e){
		for (i = 0; i < well->Number-1; i++){
			if (e->region_mark == well->region[i+1]){
				...
				p_qw = DofElementData(water->Source, e->index);
				p_p = DofElementData(oil->P, e->index);
				p_k = DofElementData(rock->perm, e->index);
				WI = 2. * M_PI * rock->DZ / (log(well->eff_radius[e->region_mark/100]/well->radius[e->region_mark/100]) );
				bar_k = Sqrt(p_k[0] * p_k[1]);
...
Test_PROD_BHP(PHASE *oil, MEDIUM *rock, WELL *well)
{
	...
	FLOAT *p_p;
	FLOAT bar_k = 0;
	FLOAT *p_qw, *p_k, *p_krw, *p_qo, *p_kro, *p_bo;
	...
	ForAllElements(g, e){
		...
		for (i = 0; i < well->Number-1; i++){
			if(e->region_mark == well->region[i+1]){
				bar_k = Sqrt(p_k[0] * p_k[1]);
				WI = 2. * M_PI * Sqrt(p_k[0] * p_k[1]) * rock->DZ / (log(well->eff_radius[e->region_mark / 100]/well->radius[e->region_mark /100]));
				lambda_o = p_kro[0] * p_bo[0] / oil->Mu;
//				if (well->PROD_BHP <=p_p[0])
					press = p_qo[0] * vol / (WI * lambda_o) + p_p[0];
//				else
//					press = p_p[0] - p_qo[0] * vol / (WI * lambda_o);
				sum[i] += press;
				count[i] += 1;
				break;
			}
...
main(int argc, char *argv[])
{
	...
	phgPrintf("Total     Time:    %lf   initial time  step:    %lf\n", control->T, control->dt);
	...
	//	time_step(control, dp, ds);
		...
		phgDofCopy(water->P, &(water_l->P), NULL, "pwl");
		...
		while (TRUE){
				/*
			if (count >= 20){
				control->dt = control->dt / 2;
				control->ct = control->pt + control->dt;
				phgPrintf("Adjust Time Step: dt = %lf\n", control->dt);
				phgPrintf("current time layer: [%lf, %lf]\n", (double)control->pt, (double)control->ct);
...
//			water_fluidity(oil, water);
			...
			Solve_Pressure(u_o, dp, ds, oil, water, oil_l, water_l, rock, rock_l, control);  
		...
		create_kr(oil, water);
		update_phi(rock, oil);
		Solver_Oileqn_So(ds, u_o, dp, oil, oil_l, rock, rock_l, control, well);
		phgDofAXPBY(1.0, ds, 1, &(oil->S));
		Disp_Saturation(oil, water);
		*/
		flag++;
