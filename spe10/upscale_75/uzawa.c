...
/*--------------------------------------------------*/
DOF *
DivRT(DOF *u, DOF *p, COM_TRO *control)
{
    GRID *g = p->g;
    SIMPLEX *e;
    DOF *B;
	...
	for (i = 0; i < N; i++) {
	    for (j = 0; j < M; j++) {
		v = -control->dt * phgQuadDivBasDotBas(e, p, i, u, j, QUAD_DEFAULT);
...
BTAB(MAT * A, DOF *B, DOF *u, DOF *p)
{
	...
	for (i = 0; i < NFace; i++) {
		...
		if (a == 0)
		{
			phgPrintf("diag of A has zero, a = %e\n", a);
		}
...
FLOAT
phgUzawa(MAT *H0, DOF *u, DOF *p, MAP *map_u, MAP *map_p, MAT *A, MAT *B, MAT *TB, MAT *C, VEC *vec_f, VEC *vec_g, INT *nits_amg)
{
